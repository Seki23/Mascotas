...
export class FormularioComponent implements OnInit {
    ...
    if(this.formularioGeneral.valid){
      ...
      console.log(this.formularioGeneral.value);
        ...
        Swal.fire({
          ...
          text:'submit disparado,,formulario es invalido',
          icon:'warning'
        });
  ...
  private llenarComboDepartamentos(){
...
muniChange(id:string):void{
  this.ubicacionPais.getDepa().pipe(map((dp)=>dp.filter((cton)=>cton.codigo.startsWith(id)&&cton.codigo.length===6))).
  subscribe((resp)=>{
    this.cantones=resp;
